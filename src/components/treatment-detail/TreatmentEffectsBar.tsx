...
function EffectIcon({ icon, size = 44 }: { icon: string; size?: number }) {
  const p = { size, color: "#9A856D", strokeWidth: 1.5 };
  switch (icon) {
    ...
    case "tooth":
      return (
        <svg width={p.size} height={p.size} viewBox="0 0 24 24" fill="none"
          stroke="#9A856D" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
export default function TreatmentEffectsBar({
    ...
    return (
      <section className="px-5 lg:px-8 py-10 lg:py-14">
        <div className="max-w-6xl mx-auto">
          <div className="bg-[#FFFFFF] rounded-2xl px-6 py-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {effects.map((eff) => (
                ...
                >
                  <div className="w-20 h-20 rounded-full bg-[#F8F3EA] flex items-center justify-center shadow-sm">
            ...
            {conclusion && (
              <div className="mt-6 bg-[#6A5542] rounded-xl px-5 py-4 text-center">
  ...
  const divideClass = count === 4 ? "" : "divide-x divide-[#D8CDBE]";

  return (
    <section className="px-5 lg:px-8 py-10 lg:py-14">
      <div className="max-w-6xl mx-auto">
        <div className="bg-[#FFFFFF] rounded-2xl px-6 pt-8 pb-7">
            ...
            {effects.map((eff) => (
              <div
                ...
                className={`flex flex-col items-center text-center px-3 py-2 gap-3 ${
                  count === 4
                    ? "border-r border-b border-[#D8CDBE] last:border-r-0 even:border-r-0 md:border-b-0 md:[&:nth-child(4)]:border-r-0"
                    : ""
                }`}
              >
                <EffectIcon icon={eff.icon} />
                <p className="text-[14px] font-extrabold text-[#2F2A26]">{eff.title}</p>
          ...
          {conclusion && (
            <div className={`bg-[#6A5542] rounded-xl px-5 py-4 text-center ${count === 4 ? "mt-6" : ""}`}>
              <p className="text-white font-extrabold text-[14px] lg:text-[15px]">
                {conclusion}
              </p>
            </div>
