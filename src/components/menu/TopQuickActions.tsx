...
const ACTIONS = [
  {
    ...
    external: true,
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#5B3A1F" strokeWidth="1.8" strokeLinecap="round">
...
export default function TopQuickActions({ onClose }: TopQuickActionsProps) {
  return (
    <div className="grid grid-cols-3 gap-3 px-4 py-4">
      {ACTIONS.map((action) => {
        const card = (
          <div className="flex flex-col items-center justify-center gap-2 bg-[#F5EEE0] rounded-xl py-4 px-2 hover:bg-[#EDE3D0] transition-colors cursor-pointer">
            {action.icon}
            <span className="text-[13px] font-semibold text-[#5B3A1F] text-center leading-tight">
              {action.label}
            </span>
          </div>
        );
