...
export default function HandwrittenNote({ review }: HandwrittenNoteProps) {
  ...
  return (
    ...
    >
      {/* 상단 핀 장식 */}
      <div className="absolute top-2 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-[#9A856D]/60 shadow-sm" />
      ...
      <div className="mt-5 mb-4 flex-1">
        {review.content.map((line, i) => (
          <p
            key={i}
            className="leading-[1.6em] text-[#9A856D]"
            style={{
      ...
      <div className="flex justify-center">
        <span className="inline-block bg-[#EFE7DC] text-[#9A856D] text-[12px] font-bold px-3 py-1 rounded-full border border-[#D8CDBE]">
          {review.category}
        </span>
      </div>
    </div>
