      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-[#FBF5E9] rounded-2xl px-6 pt-8 pb-6">
          ...
          <div className="flex items-center justify-center gap-2 mb-8">
            <LeafIcon size={14} color="#C9A567" strokeWidth={1.8} className="rotate-180" />
            <h2 className="text-[18px] lg:text-[22px] font-extrabold text-[#2A1F18]">
              항암 식단의 핵심 가치
            </h2>
            <LeafIcon size={14} color="#C9A567" strokeWidth={1.8} />
          </div>

          {/* 4개 항목 */}
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-x-0 lg:divide-x divide-[#E8DCC8]">
            {coreValues.map((val, i) => (
              <div
                key={val.title}
                className={[
                  "flex flex-col items-center text-center px-4 py-5 gap-3",
                  i % 2 === 0 && i < 2 ? "border-r border-b border-[#E8DCC8] lg:border-b-0" : "",
                  i % 2 !== 0 && i < 2 ? "border-b border-[#E8DCC8] lg:border-b-0" : "",
                  i === 1 ? "lg:border-r border-[#E8DCC8]" : "",
                  i === 2 ? "border-r border-[#E8DCC8]" : "",
                ].join(" ")}
              >
                <NutritionIcon icon={val.icon} />
                <div>
                  <p className="text-[14px] font-extrabold text-[#2A1F18] mb-1">{val.title}</p>
                  {val.description.map((line, j) => (
                    <p key={j} className="text-[12px] text-[#6B5547] leading-relaxed">{line}</p>
                  ))}
                </div>
              </div>
            ))}
