...
export default function TbtTreatmentCard({ treatment, index }: TbtTreatmentCardProps) {
  return (
    <div className="bg-[#FBF5E9] rounded-2xl overflow-hidden flex shadow-sm">
      {/* 좌측 이미지 (35%) */}
      <div className="w-[35%] flex-shrink-0 relative">
        ...
        />
        {/* 단계 번호 뱃지 */}
        <div className="absolute top-3 left-3 w-7 h-7 rounded-full bg-[#3D2817] flex items-center justify-center">
      ...
      <div className="flex-1 p-5 flex flex-col justify-center gap-2">
        <div>
          <h3 className="text-[16px] font-extrabold text-[#2A1F18] inline">{treatment.name}</h3>
          {treatment.subname && (
            <span className="text-[13px] text-[#6B5547] ml-2">{treatment.subname}</span>
          )}
        </div>
