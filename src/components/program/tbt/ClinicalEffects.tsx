    ...
    <section className="px-5 lg:px-8 py-12 lg:py-16">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          ...
          <h2 className="text-[20px] lg:text-[26px] font-extrabold text-[#2F2A26] text-center">
            임상으로 검증된 효과
          </h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {clinicalEffects.map((effect, i) => (
            <div
              key={i}
              className="bg-[#FFFFFF] rounded-2xl p-5 flex flex-col items-center text-center gap-3"
            >
              {/* 체크 원형 */}
              <div className="w-10 h-10 rounded-full bg-[#6A5542] flex items-center justify-center flex-shrink-0">
                {/* 흰색 체크 */}
              ...
              <div>
                {effect.text.map((line, j) => (
                  <p key={j} className="text-[13px] lg:text-[14px] text-[#2F2A26] font-medium leading-relaxed">
                    {line}
                  </p>
                ))}
              </div>
        ...
        <div className="bg-[#F8F2E5] rounded-xl px-5 py-4 flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-[#EFE7DC] flex-shrink-0 flex items-center justify-center">
            <TrendingUp size={20} color="#9A856D" strokeWidth={2} />
          </div>
          <p className="text-[13px] text-[#9A856D] leading-relaxed">
            다수의 항암 환자분들의 치료 결과가{" "}
